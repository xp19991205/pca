...
#这段代码对PCA和PLS进行了对比，首先根据方差（变异程度）绘制了散点图，然后以第二主成分加噪得到的数据做回归目标Y,考察PLS和PCR两种方法的预测效果
#得出的结论为当考虑两个主成分（包括第二主成分）PCR和PLS预测效果相当，而PLS只需要一个成分便能够很好的进行预测（能够去除数据间的共线性）
import numpy as np
...
y = X.dot(pca.components_[1]) + rng.normal(size=n_samples) / 2 #投影到第二主成分上，并加上噪声，作为目标预测的y
